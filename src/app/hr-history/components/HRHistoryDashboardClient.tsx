...
type SnapshotRow = { rank: number; batterId: string; batterName: string; teamId: string; opponentTeamId: string; gameId: string; predictedProbability: number; tier: string; sportsbookOddsAmerican: number | null; impliedProbability: number | null; edge: number | null; combinedScore: number | null; sportsbook: string | null; lineupConfirmed: boolean; actualHitHr: boolean | null; actualHrCount: number };
type HitPlayer = { batterId: string; batterName: string; rank: number; hrCount: number };
type Snapshot = { id: string; snapshotDate: string; boardType: BoardType; lineupMode: LineupMode; snapshotKind: string; snapshotType: SnapshotType; capturedAt: string; generatedAt: string | null; trainingStartDate: string | null; trainingExampleCount: number | null; modelTrainedAt: string | null; rowLimit: number; top5Hits: number | null; top10Hits: number | null; top15Hits: number | null; top25Hits: number | null; totalHits: number | null; scoredAt: string | null; isDeleted: boolean; deletedAt: string | null; hitPlayers: HitPlayer[]; rows: SnapshotRow[] };
type Summary = { snapshotType: SnapshotType; slateCount: number; avgTop5: number | null; avgTop10: number | null; avgTop15: number | null; avgTop25: number | null; bestDay: Snapshot | null; worstDay: Snapshot | null; rankDistribution: { top5: number; sixToTen: number; elevenToFifteen: number; sixteenToTwentyFive: number } };
...
const sortSnapshots = (a: Snapshot, b: Snapshot) => (a.boardType === b.boardType ? (typeWeight(a.snapshotType) - typeWeight(b.snapshotType)) || (new Date(b.capturedAt).getTime() - new Date(a.capturedAt).getTime()) : (a.boardType === 'model' ? 0 : a.boardType === 'best' ? 1 : 2) - (b.boardType === 'model' ? 0 : b.boardType === 'best' ? 1 : 2));
...
export default function HRHistoryDashboardClient() {
  ...
  const filtered = useMemo(() => snapshots.filter((s) => s.boardType === boardType).sort(sortSnapshots), [boardType, snapshots]);
  ...
  useEffect(() => { const p = dateSnapshots.find((s) => !s.isDeleted && s.snapshotType === 'morning_full_day') ?? dateSnapshots.find((s) => !s.isDeleted && s.snapshotType === 'pre_first_pitch') ?? dateSnapshots.find((s) => !s.isDeleted) ?? dateSnapshots[0] ?? null; setSnapshotId(p?.id ?? null); }, [dateSnapshots]);
  const selected = useMemo(() => dateSnapshots.find((s) => s.id === snapshotId) ?? dateSnapshots[0] ?? null, [dateSnapshots, snapshotId]);

  const summaries = useMemo<Summary[]>(() => Array.from(new Set(scored.map((s) => s.snapshotType))).sort((a, b) => typeWeight(a) - typeWeight(b)).map((snapshotType) => {
    const items = scored.filter((s) => s.snapshotType === snapshotType).sort(sortPerf);
    ...
    return { snapshotType, slateCount: items.length, avgTop5: avg(items.map((s) => s.top5Hits)), avgTop10: avg(items.map((s) => s.top10Hits)), avgTop15: avg(items.map((s) => s.top15Hits)), avgTop25: avg(items.filter((s) => s.rowLimit >= 25).map((s) => s.top25Hits)), bestDay: items[0] ?? null, worstDay: items.length ? items[items.length - 1] : null, rankDistribution };
  }), [scored]);

  const summaryMap = useMemo(() => new Map(summaries.map((s) => [s.snapshotType, s])), [summaries]);
  ...
  const compare = useMemo(() => {
    const morning = dateSnapshots.find((s) => !s.isDeleted && s.snapshotType === 'morning_full_day');
    const pre = dateSnapshots.find((s) => !s.isDeleted && s.snapshotType === 'pre_first_pitch');
  ...
  const selectedStrength = strength(selected, selected ? summaryMap.get(selected.snapshotType) ?? null : null);

  const handleDelete = useCallback(async (snapshot: Snapshot) => {
    const confirmed = window.confirm(
      `Delete ${typeLabel(snapshot.snapshotType)} snapshot from ${fmtDate(snapshot.snapshotDate)} captured at ${fmtAt(snapshot.capturedAt)}?`
    );
    if (!confirmed) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-slate-100">HR Validation Dashboard</h1><p className="mt-1 text-sm text-slate-400">Track saved HR boards over time, compare snapshot workflows, and inspect where home runs are landing in the rankings.</p></div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-xl border border-surface-400 bg-surface-800 p-4"><p className="text-xs text-slate-400">Hidden Deleted</p><p className="mt-1 text-xl font-bold text-amber-300">{filtered.filter((s) => s.isDeleted).length}</p></div>
      </div>
